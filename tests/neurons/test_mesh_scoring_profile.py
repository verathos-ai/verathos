...
def test_mesh_and_solo_variants_share_one_logical_emission_bucket():
    neuron = ValidatorNeuron.__new__(ValidatorNeuron)
    ...
    neuron.config = SimpleNamespace(demand_bonus_enabled=False)
    ...
    assert mesh_group == solo_group == "Qwen/Qwen2.5-7B-Instruct"
    assert neuron._last_model_group_budgets == {
        mesh_group: max(budgets.values()),
    }
