...
def test_mainnet_backend_gate_is_shared_and_testnet_unchanged():
    assert is_verathos_mainnet(netuid=96, chain_id=964)
    assert not is_verathos_mainnet(netuid=405, chain_id=945)
    assert mainnet_backend_gate_reason(
        ["CUDA0", "cuda:1"], netuid=96, chain_id=964
    ) == ""
    assert "Metal requires" in mainnet_backend_gate_reason(
        ["MTL0"], netuid=96, chain_id=964
    )
    assert "mixed CUDA/non-CUDA" in mainnet_backend_gate_reason(
        ["CUDA0", "MTL0"], netuid=96, chain_id=964
    )
    assert mainnet_backend_gate_reason(
        ["MTL0"], netuid=405, chain_id=945
    ) == ""
    assert mainnet_roster_backend_gate_reason(
        _roster(), netuid=405, chain_id=945
    ) == ""
...
def test_backend_normalization():
    ...
    assert normalize_backend("MTL0") == BACKEND_METAL
