...
class RoutingExclusion:
    """Validator-authoritative hard routing exclusion for one model slot."""
...
class ValidatorSharedState:
    ...
    # Hard per-slot policy exclusions. Unlike a score of zero, these must never
    # enter the proxy's degraded uniform-routing fallback.
    routing_excluded_entries: List[RoutingExclusion] = field(default_factory=list)
...
def write_shared_state(
    ...
    data = {
        ...
        "routing_excluded_entries": [
            ...
            for value in state.routing_excluded_entries
        ],
    ...
    try:
        ...
        routing_excluded_entries = [
            RoutingExclusion(
                ...
                model_index=int(value.get("model_index")),
                reason_code=str(value.get("reason_code") or "")[:128],
                reason=str(value.get("reason") or "")[:1024],
            )
            for value in data.get("routing_excluded_entries", [])[:16_384]
            if (
                isinstance(value, dict)
                and _EVM_ADDRESS_RE.fullmatch(str(value.get("address") or ""))
                and type(value.get("model_index")) is int
                and int(value["model_index"]) >= 0
        ...
        return ValidatorSharedState(
            ...
            routing_excluded_entries=routing_excluded_entries,
