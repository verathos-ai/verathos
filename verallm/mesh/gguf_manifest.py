...
def prewarm_proof_weight_cache_to_convergence(
    manifest: Mapping[str, Any],
    *,
    progress: Callable[[str], None] | None = None,
) -> dict[str, int]:
    """Build every required cache blob and prove the result is idempotent.

    A worker must not advertise itself as serving merely because one cache
    builder invocation returned.  Cache publication is intentionally
    best-effort at the individual file-write layer, so a write failure can
    otherwise leave a semi-warm worker whose first hard proof rebuilds a
    large Merkle tree on the audit deadline.  Run the canonical builder a
    second time and require it to perform no cache mutation.  That second
    pass is the convergence certificate used by mesh startup.
    """

    first = build_proof_weight_cache(manifest, progress=progress)
    second = build_proof_weight_cache(manifest)
    mutation_keys = ("cached", "small_f32", "merkle", "pruned")
    repeated = {
        key: int(second.get(key, 0) or 0)
        for key in mutation_keys
        if int(second.get(key, 0) or 0) != 0
    }
    if repeated:
        rendered = ", ".join(
            f"{key}={value}" for key, value in sorted(repeated.items())
        )
        raise RuntimeError(
            "proof-weight cache did not converge after its build pass: "
            + rendered
        )
    return {**first, "convergence_passes": 2}
