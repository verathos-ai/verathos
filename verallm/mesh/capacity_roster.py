...
VERATHOS_MAINNET_CHAIN_ID = 964
VERATHOS_MAINNET_NETUID = 96
MAINNET_BACKEND_REASON_PREFIX = "mainnet mesh backend gate:"
...
def normalize_backend(value: object) -> str:
    ...
    if text.startswith("cuda"):
        return BACKEND_CUDA
    if text.startswith(("metal", "mps", "mtl")):
        return BACKEND_METAL
    return BACKEND_OTHER


def is_verathos_mainnet(*, netuid: object, chain_id: object) -> bool:
    """Return whether both canonical Verathos mainnet identifiers match."""

    try:
        return (
            int(netuid) == VERATHOS_MAINNET_NETUID
            and int(chain_id) == VERATHOS_MAINNET_CHAIN_ID
        )
    except (TypeError, ValueError):
        return False


def mainnet_backend_gate_reason(
    backends: object,
    *,
    netuid: object,
    chain_id: object,
) -> str:
    """Fail closed for non-CUDA Verathos-mainnet mesh backends.

    This shared policy is used by both the operator-side deploy preflight and
    the validator's authoritative signed-roster gate. Metal remains available
    on testnet; mainnet admission requires a separate authenticated hardware
    eligibility policy.
    """

    if not is_verathos_mainnet(netuid=netuid, chain_id=chain_id):
        return ""
    if not isinstance(backends, (list, tuple, set, frozenset)) or not backends:
        return f"{MAINNET_BACKEND_REASON_PREFIX} signed GPU roster is unavailable"
    normalized = {normalize_backend(value) for value in backends}
    if normalized == {BACKEND_CUDA}:
        return ""
    if BACKEND_CUDA in normalized and len(normalized) > 1:
        return (
            f"{MAINNET_BACKEND_REASON_PREFIX} mixed CUDA/non-CUDA rosters "
            "are ineligible"
        )
    if BACKEND_OTHER in normalized:
        return f"{MAINNET_BACKEND_REASON_PREFIX} unsupported backend is ineligible"
    if BACKEND_METAL in normalized:
        return (
            f"{MAINNET_BACKEND_REASON_PREFIX} Metal requires authenticated "
            "hardware eligibility policy"
        )
    return f"{MAINNET_BACKEND_REASON_PREFIX} signed GPU roster is unavailable"


def mainnet_roster_backend_gate_reason(
    roster: object,
    *,
    netuid: object,
    chain_id: object,
) -> str:
    """Apply the mainnet backend policy to one signed-roster document."""

    if not is_verathos_mainnet(netuid=netuid, chain_id=chain_id):
        return ""
    if not isinstance(roster, Mapping):
        return mainnet_backend_gate_reason(
            None, netuid=netuid, chain_id=chain_id
        )
