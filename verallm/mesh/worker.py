...
def make_worker_server(
    *,
    ...
    if proof_trace_root is not None:
        ...
        def _proof_trace_janitor(root=proof_trace_root):
            while True:
                time.sleep(60.0)
                try:
            ...
            def _background_proof_cache_warm(manifest=proof_gguf_manifest):
                ...
                try:
                    ...
                    if free < needed + (10 << 30):
                        logger.info(
                            "skipping background proof cache warm: %.0f GB free "
                            "< %.0f GB worst-case + 10 GB headroom; hard audits "
                            "will build per draw instead",
                            free / 1e9,
                            needed / 1e9,
                        )
                        return
                    stats = build_proof_weight_cache(manifest)
                    if stats.get("cached") or stats.get("merkle"):
                        logger.info(
                            "proof weight cache warmed in background: %s", stats
                        )
                except Exception:
                    logger.warning(
                        "background proof cache warm failed; hard audits fall "
                        "back to per-draw builds",
                        exc_info=True,
                    )
