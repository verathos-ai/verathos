...
class LocalMeshRunner:
    ...
    def verify_backend_ready(self) -> str:
        ...
        self._prewarm_done.wait(self._ready_timeout_s())
        self._assert_command_active()
...
def _prewarm_proof_cache(
    ...
    *,
    ...
    """Build the driver's proof-weight blobs in the background after serving.
    ...
    the request path. Content-addressed + idempotent, so racing the serve
    process's own build-on-miss is harmless. Skips file-less members.
    """
    try:
        if entry_override is not None:
            ...
            try:
                ...
                return  # file-less member: no local GGUF to build from
        _prewarm_records(entry, stop_event, progress, model_id)
    finally:
        if done is not None:
            done.set()
...
def _prewarm_records(
    ...
    try:
        ...
        if root is None:
            return
        from verallm.mesh.gguf_manifest import _should_persist_i8

        records = _manifest_records(manifest)
        # Only warm tensors the active cache profile actually persists:
        # compact-profile tensors derive per draw by design, and treating
        # them as warm targets re-derived the whole model every launch and
        # reported it as "failed to warm N/N blobs".
        todo = [
            r for r in records
            if str(r.get("proof_i8_sha256", ""))
            and _should_persist_i8(r)
            and not _cache_blob_path(root, str(r["proof_i8_sha256"]), ".i8").is_file()
        ]
        built = 0
        failed = 0
        first_failure = ""
        for record in todo:
            if stop_event.is_set():
                return
            try:
                proof_i8_weight_matrix_from_manifest(manifest, str(record["name"]))
                cache_path = _cache_blob_path(
                    root, str(record["proof_i8_sha256"]), ".i8"
                )
                if not cache_path.is_file():
                    raise RuntimeError("proof blob was not published after build")
            except Exception as exc:
                failed += 1
                if not first_failure:
                    first_failure = str(exc)[:300]
            else:
                built += 1
            processed = built + failed
            if progress is not None and processed % 10 == 0:
                progress(
                    f"warming proofs {int(processed * 100 / max(1, len(todo)))}%"
                )
            time.sleep(0.05)  # leave CPU headroom for live generation
        if built:
            logger.info("proof-weight cache prewarmed: %d blobs for %s", built, model_id)
        if failed:
            logger.warning(
                "proof-weight cache failed to warm %d/%d blobs for %s: %s",
                failed,
                len(todo),
                model_id,
                first_failure,
